...
def batch_all_triplet_loss(labels, embeddings, margin, squared=False, epoch=0):
    ...
    # triplet_loss[triplet_loss < 0.5*margin] = 0
    # triplet_loss[triplet_loss < 0] = 0
    # triplet_loss[triplet_loss > 1.1*margin] = 0
    ...
    # print('esay:', torch.nonzero(triplet_loss == 0).size(0) - invalid)
    # print('esay:', torch.nonzero(triplet_loss <= 0.5*margin).size(0) - invalid)
    # print('hard:', torch.nonzero(triplet_loss > 0.95*margin).size(0))
    # print('hard:', torch.nonzero(triplet_loss > margin).size(0))
    # print('hard:', torch.nonzero(triplet_loss > margin).size(0)/torch.nonzero(triplet_loss > 0).size(0))
    # print('very hard:', torch.nonzero(triplet_loss > 1.5*margin).size(0)/torch.nonzero(triplet_loss > 0).size(0))
    # print(triplet_loss.size(0) * triplet_loss.size(1) * triplet_loss.size(2))

    # Remove hard triplets
    # triplet_loss[triplet_loss > margin] *= 10
